...
class MultiStackedBarChart(Graphs):
    ...
    def create_chart(self, data:dict, graph_file:str, title:str, subtitle:str, datekind:str="", kind:str="", verkoop:bool=False, belasting:dict={}, figsize:tuple=(11,7))->bool:
        try:
            ...
            p3 = ax.bar(range(len(times)), prices[:,2], bottom=prices[:,0]+prices[:,1], color=self.var_color)
            ...
            # avg_total_prices = [pd[0]+pd[1]+pd[2]+pd[3]+pd[4] for pd in price_data]
            avg_total_prices = [pd[0]+pd[1]+pd[2] for pd in price_data]
            ...
            sum_rotation_totals = 90
            sum_rotation = 0
            bar_font_size = 6
            font_size = 13
            if amount <= 8:
                sum_rotation_totals = 0
                sum_rotation = 0
                bar_font_size = 10
                font_size = 13
            ...
            for r1,r2,r3 in zip(p1,p2,p3):
                ...
                # plt.text(r1.get_x()+r1.get_width()/2., h1+h2+h3, ' € {:n}'.format(h1+h2+h3), ha='center', va='bottom', rotation=sum_rotation_totals)
                ax.text(r1.get_x()+r1.get_width()/2., h1+h2+h3, ' € {:n}'.format(h1+h2+h3), ha='center', va='bottom', rotation=sum_rotation_totals)

            for c in ax.containers:
                ax.bar_label(c, labels=[f'€ {a:n}' if a else "" for a in c.datavalues], label_type="center", rotation=sum_rotation, fontsize=bar_font_size, color='white')
            ...
            ax.legend([ 'Gem. totaal prijs', 'Gem. inkoop prijs', 'Inkoopprijs',
                        f"Vaste kosten:\nInkoopvergoeding € {belasting['opslag']:n} + EnergieBelasting € {belasting['eb']:n}",
                        f"BTW {belasting['btw']:n}%"], bbox_to_anchor=(1, 1.2), loc='upper right', prop={'size': 8})
